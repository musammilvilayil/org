  ...
  return html`<div>
    ...
    <main>
      <section id="signal" className="section"><div className="wrap"><${Reveal}><div className="kicker">01 · our signal</div><h2 className="display">The numbers are ridiculous.<br/><em>The ordinary moments are better.</em></h2><p className="lead">We talked. A lot. Some of it mattered. Some of it was complete nonsense. Both became part of the same story.</p></${Reveal}>
        <div className="stats">${[['60,617','preserved messages'],['7,717','voice notes in archive'],['1,203','sticker sends'],['91','continuous days']].map((x,i)=>html`<${motion.div} className="stat" initial=${{opacity:0,y:20}} whileInView=${{opacity:1,y:0}} viewport=${{once:true}} transition=${{delay:i*.07}}><b>${x[0]}</b><span>${x[1]}</span></${motion.div}>`)}</div>
        <p className="lead archiveNote">60,617 is the exported, preserved count — not a claim about every message ever sent. Disappearing messages had already vanished before the export, so the real lifetime conversation was larger, but the missing exact total cannot be recovered from this archive.</p>
